...
<html lang="en">
    ...
    <body>
        ...
        "/login.jsp?error=StaffLoginRequired");
        return;
        }
        ...
        <div class="container">
            ...
            <p>Viewing stock relevant to your location: <strong><c:out
                        value="${locationName}" /></strong></p>
            ...
            <table id="ownStockTable" class="display compact">
                ...
                <tbody>
                    <c:foreach var="item" items="${ownStock}">
                        <tr><td><c:out
                                    value="${item.fruitName}" /></td><td><c:out
                                    value="${item.quantity}" /></td></tr>
                    </c:foreach>
                    <c:if test="${empty ownStock}"><tr><td colspan="2">No stock
                                data found for your location.</td></tr></c:if>
                </tbody>
            </table>
            ...
            <c:if test="${isShopStaff}">
                ...
                <table id="otherShopStockTable" class="display compact">
                    <thead><tr><th>Shop Name</th><th>Fruit
                                Name</th><th>Quantity</th></tr></thead>
                    <tbody>
                        <c:foreach var="item" items="${otherShopStock}">
                            <tr><td><c:out
                                        value="${item.locationName}" /></td><td><c:out
                                        value="${item.fruitName}" /></td><td><c:out
                                        value="${item.quantity}" /></td></tr>
                        </c:foreach>
                        <c:if test="${empty otherShopStock}"><tr><td
                                    colspan="3">No stock data found for other
                                    shops in your city.</td></tr></c:if>
                    </tbody>
                </table>
            </c:if>
            ...
            <table id="centralStockTable" class="display compact">
                <thead><tr><th>Warehouse Name</th><th>Fruit
                            Name</th><th>Quantity</th></tr></thead>
                <tbody>
                    ...
                    <c:if test="${empty centralWarehouseStock}"><tr><td
                                colspan="3">No stock data found for central
                                warehouses in your country.</td></tr></c:if>
                </tbody>
            </table>
