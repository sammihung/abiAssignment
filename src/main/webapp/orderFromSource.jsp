                ...
                <tbody>
                    <c:forEach var="fruit" items="${orderableFruits}">
                        <tr>
                            ...
                            <td>
                                <%-- Add styling for low/no stock --%>
                                <c:choose>
                                    <c:when test="${fruit.availableSourceQuantity <= 0}">
                                        <span class="no-stock">0</span>
                                    </c:when>
                                    <c:when test="${fruit.availableSourceQuantity < 10}"> <%-- Example low stock threshold --%>
                            ...
                            <td>
                                <%-- Input name includes fruitId for easy parsing in servlet --%>
                                <input type="number" name="quantity_${fruit.fruitId}" min="0" value="0"
                                       ${fruit.availableSourceQuantity <= 0 ? 'disabled' : ''}> <%-- Disable if no stock --%>
                            </td>
                        </tr>
                    </c:forEach>
    ...
    <script>
        $(document).ready( function () {
            $('#orderTable').DataTable({
                "paging": false, // Disable pagination if list isn't too long
