...
<html lang="en">
    ...
    <body>
        ...
        <div class="container">
            <h1>Approve Pending Fruit Needs for <c:out
                    value="${warehouseCountry}" /></h1>
                ...
                <tbody>
                    <c:foreach var="need" items="${aggregatedNeedsList}">
                        <tr>
                            ...
                            <td><c:out
                                    value="${need.totalNeededQuantity}" /></td>
                            <td>
                                <form action="<c:url value='/needsApproval'/>"
                                    method="POST" style="display:inline;">
                                    <input type="hidden" name="fruitId"
                                        value="${need.fruitId}">
                                    <input type="hidden" name="sourceCountry"
                                        value="${need.sourceCountry}">
                    ...
                    <c:if test="${empty aggregatedNeedsList}">
                        <tr>
                            <td colspan="4">No pending needs found for your
                                warehouse's country (<c:out
                                    value="${warehouseCountry}" />).</td>
                        </tr>
                    </c:if>
                </tbody>
            </table>
